            ...
            if len(pending_orders_db) != 0 :
                for index,row in pending_orders_db.iterrows():
                    ...
                    if (row[F.entry_orderid]  not in pending_order[F.order_id].tolist()):
                        ...
                        self.database.update_one({F.entry_orderid : row[F.entry_orderid]}, { "$set": {F.entry_time : str(dt.now()), F.entry_price : entry_price, F.entry_orderid_status: F.closed, F.entry_order_execuation_type : F.limit_order, F.entry_order_count : count+1 } }) # it will update if sl hit  modificarion status to slhit
                        ...
                        if is_modified and not is_order_rejected:
                            ...
                            self.database.update_one({F.entry_orderid : order_number}, { "$set": {F.entry_price : float(market_execute_price) , F.entry_order_count: count + 1 , F.entry_order_execuation_type : F.market_order} })
            ...
            if len(pending_orders_db) != 0:
                for index,row in pending_orders_db.iterrows():
                    ...
                    if (row[F.exit_orderid]  not in pending_order[F.order_id].tolist()):
                        ...
                        self.database.update_one({F.exit_orderid : row[F.exit_orderid]}, { "$set": {F.exit_orderid_status : F.closed, F.exit_reason : F.day_end, F.exit_price : float(exit_price), F.exit_time : str(dt.now()), F.exit_order_execuation_type : F.limit_order, F.exit_order_count : count+1} } )
